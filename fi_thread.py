...
def rotate_to_90_cw(channel):
    ...
    time.sleep(0.07)  # รอให้เซอร์โวหมุนเสร็จ
...
def rotate_RBL():
    for _ in range(5):
        if _ == 0:  # รอบแรกเก็บใน data_front
            ...
            thread_up = threading.Thread(target=rotate_to_90_cw, args=(3,))
            thread_down = threading.Thread(target=rotate_to_90_ccw, args=(6,))
            thread_up.start()
            thread_down.start()
            thread_up.join()
            thread_down.join()
...
def rotate_UD():
    ...
    rotate_to_90_cw(3)
    time.sleep(5)
    thread_left = threading.Thread(target=rotate_to_90_cw, args=(4,))
    thread_right = threading.Thread(target=rotate_to_90_ccw, args=(7,))
    thread_left.start()
    thread_right.start()
    thread_left.join()
    thread_right.join()
    ...
    time.sleep(3)
    thread_left_return = threading.Thread(target=rotate_to_90_ccw, args=(4,))
    thread_right_return = threading.Thread(target=rotate_to_90_cw, args=(7,))
...
def rotate_ffiRBL():
    thread_font= threading.Thread(target=rotate_to_90_cw, args=(2,))
    thread_black = threading.Thread(target=rotate_to_90_ccw, args=(5,))
    thread_font.start()
    thread_black.start()
    thread_font.join()
    thread_black.join()
...
# หน้านึง
circle_positions = [ 
    (571, 90), (760, 110), (895,154 ),
    (551, 349),           (935, 400),
    (539, 613), (741, 621), (899, 608)  
]
#แถวบนและล่างแนวนอน
circle_positions_1 = [
   (571, 90), (760, 110), (895,154 ),
